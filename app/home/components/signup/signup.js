...
import { VarfyForm } from '@/app/global'
import eyesOpen from '@/public/svgs/eyesOpen.svg'
import eyesClose from '@/public/svgs/eyesClose.svg'
import { useDispatch } from "react-redux"
import { setUser } from "@/store/UserSlice"
export function SignUp ({ set }) {
  ...
  console.log(user)
  const createsignUpForm = async (formdata) => {
    let res
    const data = {
    ...
    if (isPrivacy) {//若勾选同意书
      res = await user.post("/api/user/register", JSON.stringify(data))
  ...
  return (
    <session>
      <div className={styles.Logincontain}>
        <div> <div className={styles.mtitle}>Join Mandarin</div>
          <div className={styles.stitle}>Be a part of Filebox community that shares the love of chinese.</div>
        </div>

        <form action={createsignUpForm} className={styles.form} id="signUp">
          ...
          <div className={styles.inputContainer}>
            <input className={styles.input}
              ...
              required={true}
              title="Password must contain at least 8 characters, including at least 1 letter and 1 number."

            />
            <Image alt="eyes" style={{ margin: "0 auto", backgroundColor: "rgba(0,0,0,0)" }} src={showPassword ? eyesOpen : eyesClose} onClick={() => {
          ...
          <div className={styles.inputContainer}>
            ...
            />
            <div className={styles.getCode}>Get Code</div>
          </div>
          ...
          <button type="submit" className={styles.loginBnt} >Sign Up</button>
          <div className={styles.signUp}>
            <div className={styles.signUpText}>{"Already have an account?"}</div>
            <div className={styles.signUpBtn} onClick={() => set("login")}>{" Log in"}</div>
          </div>
          <div className={styles.privacy}>
            <div className={styles.checkbox}>

              <div className={styles.agreeIcon} onClick={changePrivacy}>
                {isPrivacy && (<Image className={styles.yesIcon} src={checkIcon} alt="checkIcon"></Image>)}
              </div>
            </div>
